...
#     )
#     # 1. 重命名字段并添加新模型
#     models: T.List[str] = Field(
#         default_factory=lambda: DEFAULT_LLMS + ["ColbertRanker", "Flashrank", "Echorank"],
#         description="List of LLMs or dedicated reranker models to be used for reranking.",
#     )

#     def defaults(self, prefix: str = "") -> T.Dict[str, T.Any]:
#         return {
#             # 2. 更新默认参数名
#             f"{prefix}reranker_model_name": self.models[0], 
#             **self.top_k.defaults(prefix=f"{prefix}reranker_"),
#         }
...
#         return {
#             # 3. 更新分布中的参数名
#             f"{prefix}reranker_model_name": CategoricalDistribution(self.models),
#             **self.top_k.build_distributions(prefix=f"{prefix}reranker_"),
#         }

#     def get_cardinality(self) -> int:
#         # 4. 更新基数计算
#         return len(self.models) * self.top_k.get_cardinality()
...
class AgentStudyConfig(BaseSettings):
    ...
    datasets: T.List[  # type: ignore
        T.Annotated[
            T.Union[*HammerQADataset.__subclasses__()],
            Field(discriminator="xname"),
        ]
    ] = Field(
        default_factory=list,

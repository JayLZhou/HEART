...
class ColbertNodeResult(EntityResult):
    ...
    def get_node_data(self, graph, score = False):
        nodes = [graph.get_node_by_index(node_idx) for node_idx in self.node_idxs]
        if score:

            return nodes, [r for r in self.scores]
        else:
            return nodes
    def get_tree_node_data(self, graph, score = False):
  
        nodes = [graph.get_node(node_idx) for node_idx in self.node_idxs]
        if score:
...
class VectorIndexNodeResult(EntityResult):
    ...
    def get_node_data(self, graph, score = False):
        ...
        nodes = [graph.get_node(r.metadata["entity_name"]) for r in self.results]
        if score:
    ...
    def get_tree_node_data(self, graph, score = False):
        ...
        nodes = [graph.get_node(r.metadata[graph.entity_metakey]) for r in self.results]
        if score:
...
class  VectorIndexEdgeResult(RelationResult):
    ...
    def get_edge_data(self, graph, score = False):

        nodes = [graph.get_edge(r.metadata["src_id"], r.metadata["tgt_id"]) for r in self.results]
        if score:
...
class  VectorIndexSubgraphResult(SubgraphResult):
    ...
    def get_subgraph_data(self,score = False):
...
class ColbertEdgeResult(RelationResult):
    ...
    def get_edge_data(self, graph):
        return [(graph.get_edge_by_index(edge_idx), self.scores[idx]) for idx, edge_idx in enumerate(self.edge_idxs)]
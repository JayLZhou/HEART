...
from syftr.Selector import Submodular
...
@retry(
    ...
    wait=wait_exponential_jitter(
        initial=RETRY_MIN_SLEEP, max=RETRY_MAX_SLEEP, exp_base=1, jitter=RETRY_MAX_SLEEP
    ),
    reraise=True,
    retry=(
...
def eval_dataset(
    ...
    # dataset = dataset[: study_config.optimization.num_eval_samples]#The first N samples
    selector = Submodular.SubmodularSelector()
    dataset = selector.select_optimized_queries(dataset,study_config.optimization.num_eval_samples)
